  ...
  callback = function()
    vim.keymap.set("n", "<CR>", function()
      ...
      if entry and entry.type == "file" and entry.name:match("%.pdf$") then
        ...
        vim.fn.jobstart({ "sioyek", filepath }, { detach = true })
        vim.notify("Opening in sioyek: " .. entry.name)
      else
        -- Fall back to default oil behavior
        oil.select()
      end
    end, { buffer = true, desc = "Open file or navigate (PDFs → sioyek)" })
  end,
})
...
vim.keymap.set("n", "<A-t>", dap.terminate)
...
vim.api.nvim_create_autocmd("FileType", {
  ...
  callback = function(ev)
    ...
    vim.keymap.set("n", "<leader>jg", live_grep_jai_modules, opts)
    vim.keymap.set("n", "<leader>jw", grep_cursor_jai_modules, opts)
